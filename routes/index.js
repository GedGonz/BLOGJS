...
var createTransporter = require('../config/email').createTransporter;
...
async function sendMailsVerify(usuario, codverif, Destinatario) {
    var emailTransporter = await createTransporter();
...
async function sendMailsforget(email, Passworduser) {
    var emailTransporter = await createTransporter();
    await emailTransporter.sendMail({
        from: "BLOGJS <InformatiJS@BLOGJS.com>", // dirección del remitente 
...
router.post('/login/forget', function(req, res, next) {
    ...
    /******************************/
    //Envio de Correo 
    shemauser.Usuario.find({ Email: req.body.email }, async function(err, User) {
        if (await sendMailsforget(req.body.email, User[0].Password)) {
...
router.post('/login/save', multer({ storage: storage }).single('PhotoUser'), function(req, res, next) {
    ...
    cloudinary.uploader.upload(datass, function(result) {
        ...
        if (valupdate) {
            ...
            Usuariodata.save(async function(err) {
                if (!err) {
                    /******************************/
                    //Envio de Correo
                    ...
                    await sendMailsVerify(usuario, codverif, email);
